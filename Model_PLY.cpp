...
void Model_PLY::calculateNormal(float *coord1, float *coord2, float *coord3) {
    ...
    val = (float) sqrt(vr[0] * vr[0] + vr[1] * vr[1] + vr[2] * vr[2]);
...
int Model_PLY::Load(char *filename) {
    ...
    if (pch != NULL) {
        ...
        try {
            Vertex_Buffer = (float *) malloc((size_t) ftell(file));
        }
        catch (char *) {
            return -1;
        }
            ...
            for (int iterator = 0; iterator < this->TotalFaces; iterator++) {
                ...
                if (buffer[0] == '3') {
                    ...
                    */
                    ...
                    printf("%f %f %f ", Vertex_Buffer[3 * vertex1], Vertex_Buffer[3 * vertex1 + 1],
                           Vertex_Buffer[3 * vertex1 + 2]);
